...
#include "uwp_hal_sfc_hal.h"
#include "uwp_hal_aon_clk.h"

//#define LOG_INF mbed_error_printf
//#define LOG_ERR mbed_error_printf
...
static struct spi_flash_params *spiflash_scan(void)
{
	u8_t i;
	u8_t idcode[5];
	...
	//mbed_error_printf("SF: new Got idcode 0: 0x%x 1:0x%x 2:0x%x 3:0x%x 4:0x%x\n",
	...
	for (i = 0; i < ARRAY_SIZE(spi_flash_spec_table); i++) {
		...
		if (spi_spec->id_manufacturer == manufacturer_id) {
			mbed_error_printf("SF: get manufacture ID %04x\n", manufacturer_id);
			break;
		}
	}

	if (i == ARRAY_SIZE(spi_flash_spec_table)) {
		mbed_error_printf("SF: Unsupported manufacture %04x\n", manufacturer_id);
		return NULL;
	}
	...
	for (i = 0; i < spi_spec->table_num; i++) {
		...
		if (params->idcode1 == jedec) {
			if (params->idcode2 == ext_jedec)
				mbed_error_printf("SF: get ID %04x %04x\n", jedec,
						ext_jedec);
			break;
		}
	}
	...
	if (i == spi_spec->table_num) {
		mbed_error_printf("SF: Unsupported ID %04x %04x\n", jedec, ext_jedec);
		return NULL;
	} else {
		return params;
	}
		...
		u32_t data_len)
{
	...
	MBED_ASSERT(cmd_len <= 4);
	MBED_ASSERT(data_len <= 8);
		...
		u32_t data_len)
{
	...
	MBED_ASSERT(data_len <= flash->page_size);
...
static inline void spiflash_enter_xip(void)
{
...
static inline void spiflash_exit_xip(void)
{
...
void spiflash_select_xip(u32_t op)
{
	if (op == TRUE) {
		spiflash_enter_xip();
	} else {
		...
		struct spi_flash_params **params)
{
	...
	if (p == NULL) {
		mbed_error_printf("[%s] == p:[%p], scan fail!\n", __func__, p);
		return -1;
	}
