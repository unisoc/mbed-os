...
void LMecc_Generate_ECC_Key(const uint8_t* secret_key, const uint8_t* pub_key_x,
					   const uint8_t* pub_key_y,void* p_link,uint8_t blocking)
{
	...
	for (i=0;i<24;)
	{
		...
		PublicKey.x.num[(i/4)+big_num_offset] = (uint32_t)
		                                          ( (((*(pub_key_x+i    )) << 24) & 0xFF000000) +
			                                        (((*(pub_key_x+i+1  )) << 16) & 0x00FF0000) +
			                                        (((*(pub_key_x+i+2  )) <<  8) & 0x0000FF00) +
													(((*(pub_key_x+i+3  )))        & 0x000000FF));

		PublicKey.y.num[(i/4)+big_num_offset] = (uint32_t)
		                                          ( (((*(pub_key_y+i    )) << 24) & 0xFF000000) +
			                                        (((*(pub_key_y+i+1  )) << 16) & 0x00FF0000) +
			                                        (((*(pub_key_y+i+2  )) <<  8) & 0x0000FF00) +
													(((*(pub_key_y+i+3  )))        & 0x000000FF));
		i += 4;
	}
