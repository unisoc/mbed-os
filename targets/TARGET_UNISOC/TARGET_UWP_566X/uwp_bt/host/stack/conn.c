...
#if 0
static void le_conn_update(void)
{
		    ...
		    bt_conn_tx_cb_t cb)
{
    ...
    //BT_DBG("DUMP: SEND L2CAP");
    //UKI_DUMP(buf->data, buf->len);
    send_buf(conn, buf);
...
void bt_conn_notify_tx(struct bt_conn *conn)
{
	...
	while ((tx = uki_fixed_queue_dequeue(conn->tx_notify))) {
		if (tx->cb) {
			tx->cb(conn);
		}
		      ...
		      bool always_consume)
{
	...
	BT_DBG("send_frag conn %p buf %p len %u flags 0x%02x", conn, buf, buf->len,
	       flags);
	...
	if (conn->state != BT_CONN_CONNECTED) {
        BT_ERR("connection %u state error: %d", conn->handle, conn->state);
		goto fail;
	}
...
void bt_conn_process_tx(struct bt_conn *conn)
{
	...
	buf = uki_fixed_queue_dequeue(conn->tx_queue);
	BT_ASSERT(buf);
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	case BT_CONN_CONNECTED:
        ...
        conn->tx_queue = uki_fixed_queue_new();
        conn->tx_notify = uki_fixed_queue_new();
...
struct net_buf *bt_conn_create_pdu(size_t reserve)
{
	struct net_buf *buf;
