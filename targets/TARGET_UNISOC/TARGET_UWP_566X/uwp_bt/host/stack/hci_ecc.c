...
static pthread_t ecc_thread_data;
...
static void *ecc_thread(const void *p1)
{
	while (true) {
...
static void clear_ecc_events(struct net_buf *buf)
{
	...
	/*+1 for type*/
	cmd = (void *)(buf->data + 1 + sizeof(struct bt_hci_cmd_hdr));
...
int bt_hci_ecc_send(struct net_buf *buf)
{
	if (bt_buf_get_type(buf) == BT_BUF_CMD) {
		struct bt_hci_cmd_hdr *chdr = (void *)(buf->data + 1);
...
extern void *k_thread_create(const char *name, void (*thread_fn)(void *arg), void *arg, void *stack_mem, uint32_t stack_size, int priority);

void bt_hci_ecc_init(void)
{
    k_sem_init(&cmd_sem, 0, 1);
    pthread_create(&ecc_thread_data, NULL, ecc_thread, NULL);
}
