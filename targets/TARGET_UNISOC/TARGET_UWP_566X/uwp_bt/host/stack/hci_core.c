...
#include <bluetooth/bt_hw_adapter.h>
#include <bluetooth/hci.h>
//#include <nuttx/cp_init.h>

//#include "uki_sys_wrapper.h"
...
static pthread_t rx_thread_data;
//static BT_STACK_NOINIT(rx_thread_stack, CONFIG_BT_RX_STACK_SIZE);
#endif
static pthread_t tx_thread_data;
//static BT_STACK_NOINIT(tx_thread_stack, CONFIG_BT_HCI_TX_STACK_SIZE);

static pthread_attr_t tx_attr, rx_attr;
//static struct sched_param tx_param, rx_param;
...
int bt_hci_cmd_send(u16_t opcode, struct net_buf *buf)
{
	...
	//BT_DBG("send hcicmd %s(%04X) len %u", dump_cmd(opcode), opcode, buf->len);
    ...
    uki_fixed_queue_enqueue(bt_dev.cmd_tx_queue, buf);
			 ...
			 struct net_buf **rsp)
{
    ...
    //uki_mutex_lock(cmd_lock,K_FOREVER);
	...
	//BT_DBG("send hcicmd %s(%04X) len %u", dump_cmd(opcode), opcode, buf->len);
    BT_DBG("send hcicmd (%04X) len %u", opcode, buf->len);
    ...
    BT_DBG("wait hci cmd for complete");
	err = sem_wait(cmd(buf)->sync);
    sem_destroy(cmd(buf)->sync);
    pthread_mutex_unlock(&cmd_lock);
    //uki_mutex_unlock(cmd_lock);
...
static void hci_num_completed_packets(struct net_buf *buf)
{
	...
	for (i = 0; i < evt->num_handles; i++) {
		...
		while (count--) {
			...
			uki_fixed_queue_enqueue(conn->tx_notify, node);
			//sem_post(bt_conn_get_pkts(conn));
		}
...
{
	//BT_DBG("cmd %s(%04X) status: %d", dump_cmd(opcode), opcode, status);
...
static void hci_cmd_complete(struct net_buf *buf)
{
	...
	//BT_DBG("cmd %s(%04X) complete", dump_cmd(opcode), opcode);
	 ...
	 */
	status = buf->data[0];
    cmd(buf)->res = net_buf_alloc();
	net_buf_reserve(cmd(buf)->res, CONFIG_BT_HCI_RESERVE);
    memcpy(net_buf_add(cmd(buf)->res, buf->len), buf->data, buf->len);
...
static void hci_cmd_status(struct net_buf *buf)
{
	...
	//BT_DBG("cmd %s(0x%04x) complete", dump_cmd(opcode), opcode);
...
static void hci_le_meta_event(struct net_buf *buf)
{
	...
	//BT_DBG("recv le meta event %s(%02X)", dump_le_meta_event(evt->subevent), evt->subevent);
...
static void hci_event(struct net_buf *buf)
{
	...
	//BT_DBG("recv event %s(%02X)", dump_hci_event(hdr->evt), hdr->evt);

	//assert(!bt_hci_evt_is_prio(hdr->evt));
...
static void *hci_tx_thread(const void *arg)
{
	BT_DBG("hci_tx_thread Started");
...
static void read_le_features_complete(struct net_buf *buf)
{
    ...
    //BT_DBG("read_le_features_complete: ");
    //UKI_DUMP(bt_dev.le.features, sizeof(bt_dev.le.features));
}
...
static void read_buffer_size_complete(struct net_buf *buf)
{
	...
	//sem_init(&bt_dev.le.pkts, pkts, pkts);
}
#endif
...
static void read_supported_commands_complete(struct net_buf *buf)
{
	...
	//disable LE Secure for debug---begin
	//bt_dev.supported_commands[34] &= ~ 0x02;
	//bt_dev.supported_commands[34] &= ~ 0x04;
	//disable LE Secure for debug---end
}
...
static int hci_init(void)
{
	...
	err = le_init();
	if (err) {
        BT_ERR("le_init failed %s", __func__);
		return err;
	}
	...
	if (BT_FEAT_BREDR(bt_dev.features)) {
		err = br_init();
		if (err) {
            BT_ERR("br_init failed %s", __func__);
			return err;
		}
	...
	err = set_event_mask();
	if (err) {
        BT_ERR("set_event_mask failed %s", __func__);
		return err;
	}
...
static int bt_init(void)
{
	...
	err = hci_init();
	if (err) {
        BT_ERR("hci_init failed %s", __func__);
		return err;
	}
	...
	//if (IS_ENABLED(CONFIG_BT_CONN)) {
		err = bt_conn_init();
		if (err) {
            BT_ERR("bt_conn_init failed %s", __func__);
			return err;
		}
	//}
...
#endif
#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
static void *hci_rx_thread(const void *arg)
{
	struct net_buf *buf;

	BT_DBG("hci_rx_thread Started");

	while (1) {
		//BT_DBG("wait for rx incoming data");
		buf = uki_fixed_queue_dequeue(bt_dev.rx_queue);
...
static int unisoc_cmd_pskey(void)
{
	struct net_buf *buf;
	int err, size;
    char data[256] = {0};
	...
	err = bt_hci_cmd_send_sync(BT_HCI_OP_PSKEY, buf, NULL);
	if (err) {
		return err;
	}
	return 0;
...
static int unisoc_cmd_rf(void)
{
    ...
    memcpy(net_buf_add(buf, size), data, size);
    err = bt_hci_cmd_send_sync(BT_HCI_OP_RF, buf, NULL);
    if (err) {
        return err;
    }
    return 0;
...
static int unisoc_cmd_enable(void)
{
    struct net_buf *buf;
    int err, size;
    char data[256] = {0};
    ...
    memcpy(net_buf_add(buf, size), data, size);
    err = bt_hci_cmd_send_sync(BT_HCI_OP_ENABLE_CMD, buf, NULL);
    if (err) {
        return err;
    }
    return 0;
...
int bt_startup(void)
{
    ...
    bt_dev.cmd_tx_queue = uki_fixed_queue_new();
    bt_dev.rx_queue = uki_fixed_queue_new();

    BT_DBG("hci_tx_thread");
    tx_attr.stacksize = 4096;
	pthread_create(&tx_thread_data, &tx_attr, hci_tx_thread, NULL);

    BT_DBG("hci_rx_thread");
    rx_attr.stacksize = 4096;
	pthread_create(&rx_thread_data, &rx_attr, hci_rx_thread, NULL);
...
{
	struct net_buf *buf;
    unsigned char incom_type = *data;

    //UKI_DBG("DUMP RX RECV: ++");
    HCI_DUMP("<- ", data, len);

	buf = net_buf_alloc();
	net_buf_reserve(buf, CONFIG_BT_HCI_RESERVE);
    buf->len = len -1;
    memcpy(buf->data, data + 1, buf->len);

    switch (incom_type) {
        case 0x02:
            bt_buf_set_type(buf, BT_BUF_ACL_IN);
            uki_fixed_queue_enqueue(bt_dev.rx_queue, buf);
            break;
        case 0x04:
            bt_buf_set_type(buf, BT_BUF_EVT);
            if (bt_hci_evt_is_prio(*(data + 1)))
                bt_recv_prio(buf);
            else
                uki_fixed_queue_enqueue(bt_dev.rx_queue, buf);
            break;
...
struct net_buf *bt_buf_get_cmd_complete(void)
{
	...
	return bt_buf_get_rx(BT_BUF_EVT);
}
