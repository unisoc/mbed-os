...
static void sc_process_tmp(void);
...
int bt_gatt_service_register(struct bt_gatt_service *svc)
{
	...
	BT_DBG("bt_gatt_service_register ++");
	...
	if (!bt_uuid_cmp(svc->attrs[0].uuid, BT_UUID_GAP) ||
		...
		BT_DBG("bt_uuid_cmp faild");
		return -EALREADY;
	}

	err = gatt_register(svc);
	if (err < 0) {
		BT_DBG("gatt_register faild");
		return err;
	}
