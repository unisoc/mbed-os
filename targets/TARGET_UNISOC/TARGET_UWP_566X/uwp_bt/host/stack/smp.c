...
static void le_gen_dhkeys(struct bt_smp *smp);
static void le_gen_pkeys(struct bt_pub_key_cb *new_cb);
...
int bt_smp_send_security_req(struct bt_conn *conn)
{
	...
	BT_DBG("bt_smp_send_security_req");
...
{
#if defined(CONFIG_BT_KEY_GEN_IN_HOST)
	le_gen_dhkeys(smp);
#else
...
static u8_t smp_public_key(struct bt_smp *smp, struct net_buf *buf)
{
	...
	if (memcmp(smp->pkey, sc_debug_public_key, 64) == 0) {
		BT_DBG("Remote is using Debug Public key");
		atomic_set_bit(smp->flags, SMP_FLAG_SC_DEBUG_KEY);
	}
...
{
	alogo_init();
	algo_p256_generate_private_key(le_se.private_key);
	BT_DBG("%s ,private_key = %s", __func__, bt_hex(le_se.private_key, sizeof(le_se.private_key)));

	algo_p256_generate_public_key(le_se.private_key, &le_se.pk[0], &le_se.pk[32]);
	/*swap to little endian data*/
	sys_mem_swap(&le_se.pk[0], 32);
	sys_mem_swap(&le_se.pk[32], 32);
	BT_DBG("%s ,public_key = %s", __func__, bt_hex(le_se.pk, 64));

	new_cb->func(le_se.pk);
}
...
static void le_gen_dhkeys(struct bt_smp *smp)
{
	...
	atomic_set_bit(smp->flags, SMP_FLAG_DHKEY_PENDING);
	/*algo_p256_generate_dhkey uses BE data so we swap */
	sys_memcpy_swap(&rem_pk[0], &smp->pkey[0], 32);
	sys_memcpy_swap(&rem_pk[32], &smp->pkey[32], 32);

	algo_p256_generate_dhkey(le_se.private_key, &rem_pk[0],  &rem_pk[32], le_se.dhkey);
	/*swap to little endian data*/
	sys_mem_swap(le_se.dhkey, 32);
	BT_DBG("%s ,dhkey = %s", __func__, bt_hex(le_se.dhkey, 32));

	bt_smp_dhkey_ready(le_se.dhkey);
}
