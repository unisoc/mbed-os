...
static void reset_link(void)
{
	...
	if (bt_pub_key_get()) {
		atomic_set_bit(plink.flags, LOCAL_PUB_KEY);
	}
...
static int prov_auth(u8_t method, u8_t action, u8_t size)
{
	...
	case AUTH_METHOD_NO_OOB:
		...
		(void)memset(plink.auth, 0, sizeof(plink.auth));
		return 0;
	case AUTH_METHOD_STATIC:
		...
		memcpy(plink.auth + 16 - prov->static_val_len,
		       prov->static_val, prov->static_val_len);
		(void)memset(plink.auth, 0,
			     sizeof(plink.auth) - prov->static_val_len);
		return 0;
	...
	case AUTH_METHOD_OUTPUT:
		...
		if (output == BT_MESH_DISPLAY_STRING) {
			...
			(void)memset(plink.auth + size, 0,
				     sizeof(plink.auth) - size);
...
static void send_confirm(void)
{
	...
	if (bt_mesh_prov_conf(plink.conf_key, plink.rand, plink.auth,
...
static void prov_dh_key_cb(const u8_t key[32])
{
	...
	if (atomic_test_bit(plink.flags, WAIT_NUMBER) ||
	    atomic_test_bit(plink.flags, WAIT_STRING)) {
		return;
	}
...
static void prov_pub_key(const u8_t *data)
{
	...
	if (!atomic_test_bit(plink.flags, LOCAL_PUB_KEY)) {
...
static void prov_random(const u8_t *data)
{
	...
	if (bt_mesh_prov_salt(plink.conf_salt, data, plink.rand,
			      plink.prov_salt)) {
...
static void close_link(u8_t err, u8_t reason)
{
	...
	atomic_clear_bit(plink.flags, LINK_ACTIVE);
...
static void gen_prov_cont(struct prov_rx *rx, struct net_buf_simple *buf)
{
	...
	if (seg > plink.rx.last_seg) {
		...
		expect_len = (plink.rx.buf->len - 20 -
			      (23 * (plink.rx.last_seg - 1)));
...
static void gen_prov_start(struct prov_rx *rx, struct net_buf_simple *buf)
{
	...
	BT_DBG("len %u last_seg %u total_len %u fcs 0x%02x", buf->len,
	       START_LAST_SEG(rx->gpc), plink.rx.buf->len, plink.rx.fcs);
...
int bt_mesh_pb_gatt_close(struct bt_conn *conn)
{
	...
	pub_key = atomic_test_bit(plink.flags, LOCAL_PUB_KEY);
