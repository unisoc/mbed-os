...
static void bt_mesh_net_local(struct k_work *work)
{
	...
	while ((buf = uki_fixed_queue_dequeue(bt_mesh.local_queue))) {
		     ...
		     const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	if (bt_mesh_fixed_group_match(tx->ctx->addr) ||
		...
		uki_fixed_queue_enqueue(bt_mesh.local_queue, net_buf_ref(buf));
		if (cb && cb->end) {
			cb->end(0, cb_data);
		}
...
void bt_mesh_net_init(void)
{
	...
	bt_mesh.local_queue = uki_fixed_queue_new(); //uki_list_create();
