...
//NET_BUF_POOL_DEFINE(adv_buf_pool, CONFIG_BT_MESH_ADV_BUF_COUNT, BT_MESH_ADV_DATA_SIZE, BT_MESH_ADV_USER_DATA_SIZE, NULL);
static struct net_buf_pool *adv_buf_pool;
...
{
    //todo:bt_dev.hci_version
	//const s32_t adv_int_min = ((bt_dev.hci_version >= BT_HCI_VERSION_5_0) ? ADV_INT_FAST_MS : ADV_INT_DEFAULT_MS);
...
#else
	//bt_le_adv_stop();
	//bt_mesh_scan_disable();//todo:cant adv
...
static void *adv_thread(const void *p1)//, void *p2, void *p3)
{
	BT_DBG("started");
	...
	while (1) {
		...
		if (IS_ENABLED(CONFIG_BT_MESH_PROXY)) {
		    ...
		    buf = uki_list_pop_timeout(adv_queue, 0);
			while (!buf) {
				s32_t timeout;
				...
				buf = uki_list_pop_timeout(adv_queue, timeout);
				bt_mesh_proxy_adv_stop();
			}
		} else {
			//buf = net_buf_get(adv_queue, K_FOREVER);
		    buf = uki_list_pop(adv_queue);
		}
...
{
	BT_DBG("");
	//net_buf_put(adv_queue, 0);
	uki_list_insert_end(adv_queue, 0);
		      ...
		      void *cb_data)
{
	...
	uki_list_insert_end(adv_queue, net_buf_ref(buf));
}

static void bt_mesh_scan_cb(const bt_addr_le_t *addr, s8_t rssi,
			    u8_t l_adv_type, struct net_buf_simple *buf)
{
	if (l_adv_type != BT_LE_ADV_NONCONN_IND) {
		return;
	}
...
{
    pthread_t pid = 0;
    adv_queue = uki_list_create(1);
    assert(adv_queue);
    pthread_create((pthread_t *)&adv_thread_data, 0, adv_thread, 0);

    memcpy(&pid, &adv_thread_data, sizeof(pid));
    pthread_setname_np(pid, "adv_thread_data");
    pthread_detach(pid);
    /*
