...
*/
    ...
    while (uki_list_count(frnd->queue)) {
        net_buf_unref(uki_list_pop(frnd->queue));
    }

    for (i = 0; i < ARRAY_SIZE(frnd->seg); i++) {
        struct bt_mesh_friend_seg *seg = &frnd->seg[i];

        while(uki_list_count(seg->queue)) {
            net_buf_unref(uki_list_pop(seg->queue));
        }
    }
	frnd->valid = 0;
	frnd->established = 0;
...
static void enqueue_buf(struct bt_mesh_friend *frnd, struct net_buf *buf)
{
    ...
    uki_list_insert_end(frnd->queue, buf);
	frnd->queue_size++;
}
					  ...
					  u16_t src, u64_t *seq_auth)
{
	...
	for (i = 0; i < ARRAY_SIZE(frnd->seg); i++) {
		struct bt_mesh_friend_seg *seg = &frnd->seg[i];
		struct net_buf *buf = (void *)uki_list_pop(seg->queue);
			       ...
			       struct net_buf *buf)
{
	...
	uki_list_insert_end(seg->queue, buf);
	...
	if (type == BT_MESH_FRIEND_PDU_COMPLETE) {
		...
		{
		    int i = 0;
		    for (i = 0; i < uki_list_count(seg->queue); ++i)
		    {
		        buf = uki_list_find_index(seg->queue, i);
	            FRIEND_ADV(buf)->seq_auth = TRANS_SEQ_AUTH_NVAL;
	            frnd->queue_size++;
            }
		}
		...
		uki_list_merge(frnd->queue, seg->queue);
...
static void friend_timeout(struct k_work *work)
{
	...
	frnd->last = uki_list_pop(frnd->queue);
	if (!frnd->last) {
...
int bt_mesh_friend_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.frnd); i++) {
		...
		for (j = 0; j < ARRAY_SIZE(frnd->seg); j++) {
		    ...
		    frnd->seg[j].queue = uki_list_create(1);
		}
	}
...
static void friend_purge_old_ack(struct bt_mesh_friend *frnd, u64_t *seq_auth,
				 u16_t src)
{
	NUTTX_UNUSED sys_snode_t *cur, *prev = NULL;
	int i = 0;
...
#else
	for (i = 0; i < uki_list_count(frnd->queue); ++i)
	{
	    struct net_buf *buf = uki_list_find_index(frnd->queue, i);
        if (BT_MESH_ADV(buf)->addr == src &&
            FRIEND_ADV(buf)->seq_auth == *seq_auth) {

            uki_list_remove_index(frnd->queue, i);
            frnd->queue_size--;
            /* Make sure old slist entry state doesn't remain */
            buf->frags = NULL;
