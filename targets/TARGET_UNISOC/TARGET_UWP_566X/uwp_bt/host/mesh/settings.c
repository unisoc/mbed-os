...
static int NUTTX_UNUSED mesh_set(int argc, char **argv, char *val)
{
	int i;
...
static int NUTTX_UNUSED mesh_commit(void)
{
...
static void store_net_key(struct bt_mesh_subnet *sub)
{
	...
	snprintf(path, sizeof(path), "bt/mesh/NetKey/%x", sub->net_idx);
...
static void store_app_key(struct bt_mesh_app_key *app)
{
	...
	snprintf(path, sizeof(path), "bt/mesh/AppKey/%x", app->app_idx);
