 ...
 * count.
 */
#if CONFIG_BT_MESH_ADV_BUF_COUNT >= (CONFIG_BT_MESH_TX_SEG_MAX + 3)
#else
#error "CONFIG_BT_MESH_ADV_BUF_COUNT >= (CONFIG_BT_MESH_TX_SEG_MAX + 3) error"
#endif
...
{
	int i;
	seg_rx_buf_data = (typeof(seg_rx_buf_data))uki_malloc
		((CONFIG_BT_MESH_RX_SEG_MSG_COUNT * CONFIG_BT_MESH_RX_SDU_MAX));
	assert(seg_rx_buf_data);
	for (i = 0; i < ARRAY_SIZE(seg_tx); i++) {
