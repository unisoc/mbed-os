...
static void handle_interrupt_in(void)
{
    ...
    mbed_error_printf("%08x:",int_status);
...
extern int gpio_uwp_p0_init(u32_t base, PinName pin);
int gpio_irq_init(gpio_irq_t *obj, PinName pin, gpio_irq_handler handler, uint32_t id)
{
    u32_t int_type;
	...
	uwp_gpio_set_dir(obj->port_base, BIT(pin), GPIO_DIR_INPUT);
	if (obj->flags & GPIO_INT) {
		if (obj->flags & GPIO_INT_EDGE) {
			int_type = GPIO_TRIGGER_BOTH_EDGE;
		} 
		...
		uwp_gpio_int_set_type(obj->port_base, BIT(pin), int_type);
		uwp_gpio_int_clear(obj->port_base, BIT(pin));
		uwp_gpio_int_enable(obj->port_base, BIT(pin));
	}
	uwp_gpio_enable(obj->port_base, BIT(pin));
// TODO: ???
	unsigned int *p = (unsigned int*)(0x40840010);
	*p = *p | (1 << 8);
	
    uwp_gpio_input_enable(obj->port_base, BIT(pin));
...
void gpio_irq_disable(gpio_irq_t *obj){
        uwp_gpio_int_disable(obj->port_base,obj->pin_enabled);
}

void gpio_irq_enable(gpio_irq_t *obj){
        uwp_gpio_int_enable(obj->port_base,obj->pin_enabled);
}
